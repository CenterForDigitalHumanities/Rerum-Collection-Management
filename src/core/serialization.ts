...
import type { CoreEntity, RcmObject, WithContext } from "./types.js";
...
export function withContext(obj: RcmObject, context: string | Record<string, unknown> = RCM_CONTEXT): RcmObject & WithContext {
  const result = { ...obj } as RcmObject & WithContext
  if (!(result as unknown as Record<string, unknown>)["@context"]) {
    (result as unknown as Record<string, unknown>)["@context"] = context
  }
  return result
}
...
export function roundTripTest(obj: RcmObject): boolean {
  ...
  const { "@context": _, ...original } = obj as unknown as Record<string, unknown>
  const { "@context": __, ...restored } = parsed as unknown as Record<string, unknown>
