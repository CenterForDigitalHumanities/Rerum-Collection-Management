...
import type { CoreEntity, Thing, Annotation } from "../core/types";
...
export class GraphEngineImpl implements GraphEngine {
  ...
  assembleChonkyNode(thingId: string): ChonkyNode {
    ...
    const body = annotations[0]?.["oa:hasBody"];
  ...
  private traverse(
    ...
    if (node) {
      if (!options.typeFilter || this.typeMatches(node.entity, options.typeFilter)) {
        nodes.push(node);
      }
    }
  ...
  private extractTargetId(annotation: Annotation): string | undefined {
    ...
    if (typeof target === "object" && target !== null) {
      return (target as Record<string, unknown>)["source"] as string | undefined;
    }
    return undefined;
  }
