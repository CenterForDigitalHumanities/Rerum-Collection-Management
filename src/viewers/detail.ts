...
export class DetailViewer implements Viewer {
  ...
  render(node: ChonkyNode, options?: RenderOptions): ViewerOutput {
    ...
    if (node.humanAnnotations.length > 0) {
      ...
      for (const ann of node.humanAnnotations) {
        html += `<li class="assertion">
          ...
          ${showEvidence && ann["rcm:evidence"] ? `<details><summary>Evidence</summary><ul>${ann["rcm:evidence"].map((e: string) => `<li><a href="${this.escapeHtml(e)}">${this.escapeHtml(e)}</a></li>`).join("")}</ul></details>` : ""}
        </li>`
      }
      html += `</ul></section>`
    }
    ...
    if (node.representations.length > 0) {
      html += `<section class="representations">
        ...
        <ul>${node.representations.map((r: string) => `<li><a href="${this.escapeHtml(r)}">${this.escapeHtml(r)}</a></li>`).join("")}</ul>
      </section>`
    }
    ...
    if (node.machineAnnotations.length > 0) {
      html += `<section class="machine-suggestions">
        ...
        <ul>${node.machineAnnotations.map((a: import("../core/types.js").Annotation) => `<li>${this.escapeHtml(JSON.stringify(a["oa:hasBody"]))}</li>`).join("")}</ul>
      </section>`
    }
    ...
    if (showDisputes && node.disputes.length > 0) {
      html += `<section class="disputes">
        ...
        <ul>${node.disputes.map((a: import("../core/types.js").Annotation) => `<li>${this.escapeHtml(JSON.stringify(a["oa:hasBody"]))}</li>`).join("")}</ul>
      </section>`
    }
    ...
    if (node.collections.length > 0) {
      html += `<section class="collections">
        ...
        <ul>${node.collections.map((c: string) => `<li><a href="${this.escapeHtml(c)}">${this.escapeHtml(c)}</a></li>`).join("")}</ul>
      </section>`
    }
    ...
    if (node.relatedThings.length > 0) {
      html += `<section class="related">
        ...
        <ul>${node.relatedThings.map((e: { source: string; predicate: string; target: string }) => `<li><a href="${this.escapeHtml(e.target)}">${this.escapeHtml(e.predicate)} → ${this.escapeHtml(e.target)}</a></li>`).join("")}</ul>
      </section>`
    }
  ...
  renderSlice(slice: GraphQueryResult, options?: RenderOptions): ViewerOutput {
    const html = `<div class="rcm-viewer rcm-viewer-detail">
      ...
      <ul class="item-list">
        ${slice.nodes.map((n: import("../graph/types.js").GraphNode) => `<li><a href="${this.escapeHtml(n.entity["@id"])}">${this.escapeHtml(String((n.entity as unknown as Record<string, unknown>)["rdfs:label"] ?? n.entity["@id"]))}</a></li>`).join("")}
      </ul>
    </div>`
